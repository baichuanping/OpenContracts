...
- **System Settings page failed to load with `Enum 'FileTypeEnum' cannot represent value: 'md'`** (`config/graphql/pipeline_types.py:17`): the GraphQL `FileTypeEnum` was missing the `MD` member, but the backend `FileTypeEnum` has `MD = "md"` and `oc_markdown_parser.py` declares `supported_file_types = [FileTypeEnum.MD]`. When the pipeline-components query serialized the markdown parser's supported file types, the GraphQL enum could not represent `"md"` and the entire query failed. Added `MD = BackendFileTypeEnum.MD.value` so the GraphQL enum stays in sync with the backend source of truth.
