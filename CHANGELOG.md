...
- **Phase F — authorization-invariant gap-fill + permissioning docs refresh** (issue #1660; final phase of the #1637 permission-centralization plan). Closes the remaining gaps in the `user_can` invariant suite and re-points the permissioning docs at the new API.
  ...
  - **Audit finding — group-perm filter/check drift.** The audit's group-shared coverage surfaced a genuine per-model drift: `Manager.user_can` honours group object-permissions (`_default_user_can` resolves with `include_group_permissions=True`) but `QuerySet.visible_to_user` in `opencontractserver/shared/QuerySets.py` only consults the *user* object-permission table, never the `*groupobjectpermission` table. `Corpus` is unaffected (its queryset handles groups); `Document`, `Annotation`, and `Note` drift. The current behaviour is pinned by `DocumentAuthorizationInvariantsTestCase.test_group_shared_known_drift_user_can_vs_visible_to_user` so it cannot change silently. Closing the drift is a query-path behaviour change left out of this test/docs-only sweep.
...
- **Test suite optimization Phase 2 — convert 160 needlessly-`TransactionTestCase` classes to `TestCase`** (issue #1710; 28 files under `opencontractserver/tests/` and `opencontractserver/tests/permissioning/`). `TransactionTestCase` `TRUNCATE`s every table and re-emits the `post_migrate` signal (`create_permissions`, `create_contenttypes`, django-guardian setup) between *every* test method; `TestCase` instead wraps each test in a transaction and rolls it back — far cheaper per-test overhead, and the highest-leverage structural lever for bringing the ~1 hr backend suite under 20 minutes. All 235 classes that extend `TransactionTestCase` directly were audited against the patterns that genuinely require transaction semantics, and the 160 pure-synchronous ORM / GraphQL / permission / management-command classes were reclassified to `django.test.TestCase`.
