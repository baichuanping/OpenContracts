...
7. **Corpus / document access — always via the service layer.** Code with a user context (request handlers, GraphQL resolvers, MCP tools, LLM tools, REST views, Celery tasks invoked with a user) **must** reach documents through a service:
   ...
   - **Corpus-scoped operations** ("give me X inside corpus Y for user Z") — covered today by documents-in-corpus and folders, growing to annotations, notes, relationships, extracts, and analyses. **Phase 2A (#1716) split the former `CorpusObjsService` monolith into the segmented `opencontractserver/corpuses/services/` package — prefer the segmented services for new code:** `CorpusDocumentService` (document-in-corpus reads/writes + membership), `FolderCRUDService` (folder CRUD, tree, search), `FolderDocumentService` (document-in-folder placement), `DocumentLifecycleService` (soft-delete/restore/trash), `CorpusPathService` (`DocumentPath` disambiguation internals). `CorpusObjsService` in `opencontractserver/corpuses/corpus_objs_service.py` remains as a deprecated re-export facade for existing callers and will be removed once they migrate (Phase 2C); do not point new code at it.
